...
public:
    ...
    void print_coefficients() const {
        ...
        for (int i = 0; i < NUMBER_OF_BIQUAD_BLOCKS; ++i) {
            std::cout << "b" << i << "0: " << _coefficients[i]
    ...
    bool push_biquad_coefficients(U b0, U b1, U b2, U a1, U a2) {
        ...
        } else if constexpr (std::is_same<T, q31_t>::value && std::is_same<T, double>::value) {
        ...
        } else if constexpr (std::is_same<T, q31_t>::value && std::is_same<T, float>::value) {
